...
// === ログ抑制設定 ===
// Chromium内部のWarning/Errorログ（Unsupported pixel formatなど）を抑制
...
function getIconPath() {
    const iconFile = process.platform === 'darwin' ? 'VSL.png' : 'VSL.ico';
    if (app.isPackaged) {
...
function resolveProdPathMethod3() {
    try {
        ...
        const normalizedPlatform = platformMap[process.platform] || process.platform;
        ...
        const template = [
            {
                label: 'ファイル',
                submenu: [
                    { label: 'プロジェクトを開く...', accelerator: 'Ctrl+O', click: () => { mainWindow.webContents.send('project-open'); } },
                    { label: 'プロジェクトを保存...', accelerator: 'Ctrl+S', click: () => { mainWindow.webContents.send('project-save'); } },
                    { type: 'separator' },
                    { label: '動画1を開く', accelerator: 'Ctrl+1', click: () => { mainWindow.webContents.send('open-video', 'left'); } },
                    { label: '動画2を開く', accelerator: 'Ctrl+2', click: () => { mainWindow.webContents.send('open-video', 'right'); } },
                    { type: 'separator' },
                    ...
                    { type: 'separator' },
                    { label: '動画を閉じる', accelerator: 'Ctrl+W', click: () => { mainWindow.webContents.send('close-video'); } },
                    { type: 'separator' },
